...
const createGetHandler = (tableName: string, orderBy?: string) => {
...
const createGetSingleHandler = (tableName: string) => {
    return async (req: any, res: any) => {
        try {
            const db = await getDb();
...
const createPostHandler = (tableName: string) => {
    return async (req: any, res: any) => {
        try {
            const db = await getDb();
            ...
            console.log(`[${tableName}] Inserting:`, req.body);
...
const createPutHandler = (tableName: string) => {
    return async (req: any, res: any) => {
        try {
            const db = await getDb();
            const { id } = req.params;
...
const createDeleteHandler = (tableName: string) => {
    return async (req: any, res: any) => {
        try {
            const db = await getDb();
            const { id } = req.params;
    ...
    try {
        ...
        const mappedData = data.map((p: any) => {
            ...
            try {
                ...
                // If not JSON, assumy legacy or empty
            }
...
router.post('/projects', async (req, res) => {
    try {
        ...
        // We still save category string for redundancy? No, strictly use category_id, but current schema has category string column.
        // We should keep category string populated for safety if other parts use it, or just use categoryName.
        // The instruction was "projects (add category_id or equivalent)". I added category_id.
        // I should stick to filling the legacy column too if it remains not null.

        const result = await db.run(query, [
            title,
            location,
            category_id,
            categoryName, // Filling legacy column
            description || '',
            imageUrl,
            imagesJson,
...
router.post('/testimonials', createPostHandler('testimonials'));
router.put('/testimonials/:id', createPutHandler('testimonials'));
...
router.get('/dashboard-tasks', async (req, res) => {
    try {
        ...
        inactiveServices.forEach((s: any) => {
            tasks.push({
                id: `service-${s.id}`,
                type: 'Service',
                title: s.title,
